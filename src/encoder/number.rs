...
pub trait EncodeUnknownValue {
    /// Encodes the value using the provided [`NumberEncoder`].
    fn encode(&self, encoder: &mut dyn NumberEncoder) -> fmt::Result;
}
...
pub trait EncodeGaugeValue {
    /// Encodes the gauge value using the provided [`NumberEncoder`].
    fn encode(&self, encoder: &mut dyn NumberEncoder) -> fmt::Result;
}
...
pub trait EncodeCounterValue {
    /// Encodes the counter value using the provided [`NumberEncoder`].
    fn encode(&self, encoder: &mut dyn NumberEncoder) -> fmt::Result;
}
